...
print('============================')
print('Current test:')
print('- L2 Regularization added with 0.001 penalty factor.')
print('- Use tanh activation function in Dense layer.')
print('- Using test_dataset folder for data.')
print('============================')
...
# Split the data for training in 99% train, to use almost all data for training
# Then use the data that is the directory /test_dataset for validation and testing
train_df_new, temp_df_new = train_test_split(
df_resampled,
train_size=0.99, # Split 99% for training to utilize more data
shuffle=True,
random_state=42,
...
# Load test dataset from the specified test_dataset folder
...
test_dataset_gen = ts_gen.flow_from_dataframe(
    test_dataset_df,
    x_col='image_path',
    y_col='category_encoded',
    target_size=img_size,
...
def create_xception_model(input_shape, num_classes=8, learning_rate=1e-4):
    ...
    # Add Dense layaers with L2 regularization.
    ...
    x = Dense(512, activation="tanh", kernel_regularizer=regularizers.l2(0.001))(x)    
...
# Use the test dataset from the test_dataset folder 
# to run predictions and evaluate the model
test_labels = test_dataset_gen.classes
predictions = cnn_model.predict(test_dataset_gen)

predicted_classes = np.argmax(predictions, axis=1)

# Displays classification report, which includes f1-score, etc.
report = classification_report(test_labels, predicted_classes,
target_names=list(test_dataset_gen.class_indices.keys()))
...
xticklabels=list(test_dataset_gen.class_indices.keys()),
yticklabels=list(test_dataset_gen.class_indices.keys()))
