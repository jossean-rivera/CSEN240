...
print('- **Updated data loading to use separate train, val, and test folders**')
print('=============================================')
...
# New function to handle loading from separate directories
def load_data_from_dir(base_path, categories):
    image_paths = []
    labels = []
...
valid_df = pd.DataFrame({"image_path": valid_image_paths, "label": valid_labels})
test_df = pd.DataFrame({"image_path": test_image_paths, "label": test_labels})

if train_df.empty:
    print("Training DataFrame is empty. Cannot proceed.")
else:
    print('Train Data Frame dimensions:', train_df.shape)
    print('Validation Data Frame dimensions:', valid_df.shape)
    print('Test Data Frame dimensions:', test_df.shape)


# In[5]:


# --- Inspect Training Data (Primary set) ---
if not train_df.empty:
    print('--- Training Data Info ---')
    print('Dupplicated sum:', train_df.duplicated().sum())
    print('IsNull sum:')
    print(train_df.isnull().sum())
    print('Data Frame Info')
    train_df.info()

    print("Unique labels: {}".format(train_df['label'].unique()))
    print("Label counts: {}".format(train_df['label'].value_counts()))
...
if not train_df.empty:
    ...
    ax.set_title("Distribution of Tumor Types (Train)", fontsize=14, fontweight='bold')
...
if not train_df.empty:
    ...
    for i, category in enumerate(categories):
        # Use train_df for visualization
        category_images = train_df[train_df['label'] == category]['image_path'].iloc[:num_images]
...
# --- Apply Label Encoding to all DataFrames ---
if not train_df.empty:
    label_encoder = LabelEncoder()
    # Fit on training data only to define the encoding
    train_df['category_encoded'] = label_encoder.fit_transform(train_df['label'])

    # Transform validation and test data using the fitted encoder
    valid_df['category_encoded'] = label_encoder.transform(valid_df['label'])
    test_df['category_encoded'] = label_encoder.transform(test_df['label'])

    # Set processed dataframes for next steps
    train_df_processed = train_df[['image_path', 'category_encoded']]
    valid_df_processed = valid_df[['image_path', 'category_encoded']]
    test_df_processed = test_df[['image_path', 'category_encoded']]
else:
    # Set empty processed dataframes if train_df was empty
    train_df_processed = pd.DataFrame(columns=['image_path', 'category_encoded'])
    valid_df_processed = pd.DataFrame(columns=['image_path', 'category_encoded'])
    test_df_processed = pd.DataFrame(columns=['image_path', 'category_encoded'])
...
if 'train_df_processed' in locals() and not train_df_processed.empty:
    ros = RandomOverSampler(random_state=42)
    X_resampled, y_resampled = ros.fit_resample(train_df_processed[['image_path']],
    train_df_processed['category_encoded'])

    # Final training DataFrame is the resampled one
    train_df_new = pd.DataFrame(X_resampled, columns=['image_path'])
    train_df_new['category_encoded'] = y_resampled

    # Final validation and test DataFrames are the pre-split, non-oversampled ones
    valid_df_new = valid_df_processed
    test_df_new = test_df_processed

    print("\nClass distribution after oversampling (Train Set):")
    print(train_df_new['category_encoded'].value_counts())
    print(f"Final Train Shape: {train_df_new.shape}")
    print(f"Final Validation Shape: {valid_df_new.shape}")
    print(f"Final Test Shape: {test_df_new.shape}")
else:
    print("Skipping resampling as initial Training DataFrame was empty.")
    # Ensure final DataFrames are empty to skip training later
...
# --- Data Split is now handled by the separate folder structure, confirming shapes ---
if not train_df_new.empty:
    ...
    print("Skipping data confirmation as training DataFrame is empty.")
